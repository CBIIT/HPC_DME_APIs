...
public class HpcIRODSConnection {
	...
	private HpcIRODSConnection(String irodsHost, Integer irodsPort, String irodsZone, String irodsResource,
		...
		try {
			...
			throw new HpcException("Failed to instantiate iRODs file system: " + e.getMessage(),
	...
	public IRODSFileFactory getIRODSFileFactory(Object authenticatedToken) throws HpcException {
		try {
			...
			throw new HpcException("Failed to get iRODs file factory instance: " + e.getMessage(),
					HpcErrorType.DATA_MANAGEMENT_ERROR, e);
		}
	}
	...
	public CollectionAO getCollectionAO(Object authenticatedToken) throws HpcException {
		try {
			return irodsFileSystem.getIRODSAccessObjectFactory().getCollectionAO(getIrodsAccount(authenticatedToken));

		} catch (JargonException e) {
			throw new HpcException("Failed to get iRODs Colelction Access Object: " + e.getMessage(),
					HpcErrorType.DATA_MANAGEMENT_ERROR, e);
		}
	}
	...
	public DataObjectAO getDataObjectAO(Object authenticatedToken) throws HpcException {
		try {
			return irodsFileSystem.getIRODSAccessObjectFactory().getDataObjectAO(getIrodsAccount(authenticatedToken));

		} catch (JargonException e) {
			throw new HpcException("Failed to get iRODs Data Object Access Object: " + e.getMessage(),
					HpcErrorType.DATA_MANAGEMENT_ERROR, e);
		}
	}
	 ...
	 * @throws HpcException on iRODS failure.
	 */
	public CollectionAndDataObjectListAndSearchAO getCollectionAndDataObjectListAndSearchAO(Object authenticatedToken)
			throws HpcException {
		try {

			SettableJargonProperties props = new SettableJargonProperties(
					irodsFileSystem.getIrodsSession().getJargonProperties());
	...
	public UserAO getUserAO(Object authenticatedToken) throws HpcException {
		try {
			return irodsFileSystem.getIRODSAccessObjectFactory().getUserAO(getIrodsAccount(authenticatedToken));

		} catch (JargonException e) {
			throw new HpcException("Failed to get iRODs User Access Object: " + e.getMessage(),
					HpcErrorType.DATA_MANAGEMENT_ERROR, e);
		}
	}
	...
	public UserGroupAO getUserGroupAO(Object authenticatedToken) throws HpcException {
		try {
			return irodsFileSystem.getIRODSAccessObjectFactory().getUserGroupAO(getIrodsAccount(authenticatedToken));

		} catch (JargonException e) {
			throw new HpcException("Failed to get iRODs User Group Access Object: " + e.getMessage(),
					HpcErrorType.DATA_MANAGEMENT_ERROR, e);
		}
	}
	 ...
	 * @return An authenticated IRODSAccount object, or null if authentication
	 *         failed.
	 * @throws HpcException on iRODS failure.
	 */
	public IRODSAccount authenticate(HpcIntegratedSystemAccount dataManagementAccount,
		...
		try {
			if (authenticationType.equals(HpcAuthenticationType.TOKEN)
				...
				// The data management account was previously authenticated. Return an
				// authenticated iRODS account.
				...
				AuthResponse authResponse = irodsFileSystem.getIRODSAccessObjectFactory().authenticateIRODSAccount(
				...
				if (authResponse != null) {
					...
					logger.info("IRODS account authenticated: {} [Scheme requested: {}, Scheme used: {} ]",
							irodsAccount.getUserName(), authenticationScheme, irodsAccount.getAuthenticationScheme());
				}
			}
			...
			throw new HpcException(
					...
					HpcErrorType.DATA_MANAGEMENT_ERROR, HpcIntegratedSystem.IRODS, ae);

		} catch (JargonException e) {
			throw new HpcException("Failed to authenticate an iRODS account: " + dataManagementAccount.getUsername()
					+ ", " + e.getMessage(), HpcErrorType.DATA_MANAGEMENT_ERROR, HpcIntegratedSystem.IRODS, e);
	 ...
	 * @param authScheme            The iRODS authentication scheme (PAM or
	 *                              STANDARD).
	...
	private IRODSAccount toUnauthenticatedIrodsAccount(HpcIntegratedSystemAccount dataManagementAccount,
			AuthScheme authScheme) throws HpcException {
		try {
			IRODSAccount irodsAccount = IRODSAccount.instance(irodsHost, irodsPort, dataManagementAccount.getUsername(),
					this.pamAuthentication ? dataManagementAccount.getPassword()
							: getUserKey(dataManagementAccount.getUsername()),
					"", irodsZone, irodsResource);
			irodsAccount.setAuthenticationScheme(authScheme);
			return irodsAccount;

		} catch (JargonException e) {
			throw new HpcException("Failed instantiate an iRODS account: " + e.getMessage(),
					HpcErrorType.DATA_MANAGEMENT_ERROR, e);
		} catch (NoSuchAlgorithmException e) {
			throw new HpcException("Failed instantiate an iRODS account: " + e.getMessage(),
					HpcErrorType.DATA_MANAGEMENT_ERROR, e);
	...
	private IRODSAccount toAuthenticatedIrodsAccount(HpcIntegratedSystemAccount dataManagementAccount,
			AuthScheme authScheme) throws JargonException, NoSuchAlgorithmException {
		...
		return IRODSAccount.instance(properties.get(HOST_PROPERTY), Integer.valueOf(properties.get(PORT_PROPERTY)),
				...
				pamAuthentication ? dataManagementAccount.getPassword()
						: getUserKey(dataManagementAccount.getUsername()),
				properties.get(HOME_DIRECTORY_PROPERTY), properties.get(ZONE_PROPERTY),
				properties.get(DEFAULT_STORAGE_RESOURCE_PROPERTY), authScheme);
	}
	...
	private IRODSAccount getIrodsAccount(Object authenticatedToken) throws HpcException {
		if (authenticatedToken == null || !(authenticatedToken instanceof IRODSAccount)) {
			throw new HpcException("Invalid iRODS authentication token", HpcErrorType.INVALID_REQUEST_INPUT);
		}
