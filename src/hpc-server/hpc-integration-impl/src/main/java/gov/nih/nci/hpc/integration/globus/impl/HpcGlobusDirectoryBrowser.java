...
public class HpcGlobusDirectoryBrowser {
   ...
   * immediately under the directory. No traversing / recursive calls to sub-directories. See:
   ...
   */
  public Result list(HpcFileLocation dirLocation, JSONTransferAPIClient client) throws Exception {
    ...
    return retryTemplate.execute(
        arg0 -> {
          String resource =
              BaseTransferAPIClient.endpointPath(dirLocation.getFileContainerId()) + "/ls";
          return client.getResult(resource, params);
        });
  }

  /**
   * Scan a Globus directory. Each file in the directory tree structure (including all sub
   * directories recursively) is visited.
   *
