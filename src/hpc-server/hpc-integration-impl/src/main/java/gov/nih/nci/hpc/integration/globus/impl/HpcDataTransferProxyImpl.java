...
public class HpcDataTransferProxyImpl implements HpcDataTransferProxy {
	...
	@Override
	public Object authenticate(HpcIntegratedSystemAccount dataTransferAccount, String urlNotUsed) throws HpcException {
		// Note: At this time, there is no Globus config items in data management
		// configuration DB.
		// The Globus connection URL is configured via Spring. In the future, this may
		// be
	...
	public HpcDataObjectUploadResponse uploadDataObject(Object authenticatedToken,
		...
		String requestId = transferData(client, uploadRequest.getSourceLocation(), archiveDestinationLocation);
	...
	public HpcDataObjectDownloadResponse downloadDataObject(Object authenticatedToken,
		...
		// Submit a request to Globus to transfer the data.
		response.setDataTransferRequestId(transferData(globusConnection.getTransferClient(authenticatedToken),
				downloadRequest.getArchiveLocation(), downloadRequest.getDestinationLocation()));
	...
	@Override
	public long getDataTransferSize(Object authenticatedToken, String dataTransferRequestId) throws HpcException {
	...
	public String getFilePath(String fileId, boolean archive) throws HpcException {
		return archive
				? fileId.replaceFirst(baseArchiveDestination.getFileLocation().getFileId(),
		...
		try {
			...
			if (error.statusCode == 502) {
				if (error.code.equals(NOT_DIRECTORY_GLOBUS_CODE)) {
					...
					throw new HpcException("Invalid file location:" + fileLocation, HpcErrorType.INVALID_REQUEST_INPUT,
							error);
				}
	 ...
	 *             on data transfer system failure.
	 */
	private void cancelTransferRequest(Object authenticatedToken, String dataTransferRequestId) throws HpcException {
		JSONTransferAPIClient client = globusConnection.getTransferClient(authenticatedToken);
