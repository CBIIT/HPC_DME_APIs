...
public class HpcDataTransferProxyImpl implements HpcDataTransferProxy {
	 ...
	 * @Autowired
	 * 
	 * @Qualifier("hpcGoogleDriveDownloadExecutor") Executor googleDriveExecutor =
	 * null;
	 */
	 ...
	 * 
	 * // The logger instance. private final Logger logger =
	...
	public String downloadDataObject(Object authenticatedToken, HpcDataObjectDownloadRequest downloadRequest,
		...
		/*
		 * // Input validation if (progressListener == null) { throw new HpcException(
		 * "[Google Drive] No progress listener provided for a download to Google Drive destination"
		 * , HpcErrorType.UNEXPECTED_ERROR); }
		 * 
		 * // Authenticate the Google Drive access token. final Drive drive =
		 * googleDriveConnection.getDrive(
		 * googleDriveConnection.authenticate(downloadRequest.getGoogleDriveDestination(
		 * ).getAccessToken()));
		 * 
		 * // Stream the file to Google Drive. CompletableFuture<Void>
		 * googleDriveDownloadFuture = CompletableFuture.runAsync(() -> { try { // Find
		 * / Create the folder in Google Drive where we download the file to String
		 * destinationPath = toNormalizedPath(
		 * downloadRequest.getGoogleDriveDestination().getDestinationLocation().
		 * getFileId()); int lastSlashIndex = destinationPath.lastIndexOf('/'); String
		 * destinationFolderPath = destinationPath.substring(0, lastSlashIndex); String
		 * destinationFileName = destinationPath.substring(lastSlashIndex + 1); String
		 * folderId = getFolderId(drive, destinationFolderPath, true);
		 * 
		 * // Transfer the file to Google Drive, and complete the download task.
		 * progressListener.transferCompleted(drive.files() .create(new
		 * File().setName(destinationFileName).setParents(Collections.singletonList(
		 * folderId)), new InputStreamContent("application/octet-stream", new
		 * URL(downloadRequest.getArchiveLocationURL()).openStream()))
		 * .setFields("size").execute().getSize());
		 * 
		 * } catch (IOException e) { String message =
		 * "[GoogleDrive] Failed to download object: " + e.getMessage();
