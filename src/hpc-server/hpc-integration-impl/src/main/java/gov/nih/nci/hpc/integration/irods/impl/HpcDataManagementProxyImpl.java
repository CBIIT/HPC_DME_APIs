...
public class HpcDataManagementProxyImpl implements HpcDataManagementProxy {
  ...
  public void createCollectionDirectory(Object authenticatedToken, String path)
    ...
    try {
      ...
      throw new HpcException("Failed to create a collection directory: " + e.getMessage(),
  ...
  @Override
  public void setCollectionPermissionInheritace(Object authenticatedToken, String path,
      boolean permissionInheritace) throws HpcException {
    try {
      irodsConnection.getCollectionAO(authenticatedToken).setAccessPermissionInherit(
  ...
  @Override
  public void addMetadataToCollection(Object authenticatedToken, String path,
    ...
    try {
      ...
      throw new HpcException("Failed to add metadata to a collection: " + e.getMessage(),
          HpcErrorType.DATA_MANAGEMENT_ERROR, HpcIntegratedSystem.IRODS, e);
    }
  }

  @Override
  public void updateCollectionMetadata(Object authenticatedToken, String path,
      List<HpcMetadataEntry> metadataEntries) throws HpcException {
    try {
      ...
      throw new HpcException("Failed to update collection metadata: " + e.getMessage(),
          HpcErrorType.DATA_MANAGEMENT_ERROR, HpcIntegratedSystem.IRODS, e);
    }
  }

  @Override
  public void addMetadataToDataObject(Object authenticatedToken, String path,
      List<HpcMetadataEntry> metadataEntries) throws HpcException {
    List<AvuData> avuDatas = new ArrayList<AvuData>();
  ...
  @Override
  public void updateDataObjectMetadata(Object authenticatedToken, String path,
      List<HpcMetadataEntry> metadataEntries) throws HpcException {
    try {
      String absolutePath = getAbsolutePath(path);
      ...
      for (HpcMetadataEntry metadataEntry : metadataEntries) {
        ...
        try {
          ...
          // Metadata was not found to update. Add it.
          irodsConnection.getDataObjectAO(authenticatedToken).addAVUMetadata(absolutePath, avuData);
        }
      }
  ...
  public boolean interrogatePathRef(Object authenticatedToken, String path) throws HpcException {
    IRODSFile file = null;
    try {
      file = irodsConnection.getIRODSFileFactory(authenticatedToken)
          .instanceIRODSFile(getAbsolutePath(path));
      if (file.exists()) {
      ...
      throw new HpcException(String.format(
          ...
          path), HpcErrorType.INVALID_REQUEST_INPUT, e);
    ...
    try {
      return toHpcCollection(
          ...
          list ? irodsConnection.getCollectionAndDataObjectListAndSearchAO(authenticatedToken)
              .listDataObjectsAndCollectionsUnderPath(getAbsolutePath(path)) : null);

    } catch (Exception e) {
      throw new HpcException("Failed to get Collection: " + path + ". " + e.getMessage(),
  ...
  public List<HpcMetadataEntry> getCollectionMetadata(Object authenticatedToken, String path)
    ...
    try {
      ...
      throw new HpcException(
          "Failed to get metadata of a collection: " + path + ". " + e.getMessage(),
          HpcErrorType.DATA_MANAGEMENT_ERROR, null, e);
    }
  }
  ...
  public HpcDataObject getDataObject(Object authenticatedToken, String path) throws HpcException {
    try {
      ...
      throw new HpcException("Failed to get Data Object: " + e.getMessage(),
          HpcErrorType.DATA_MANAGEMENT_ERROR, null, e);
    }
  }
  ...
  public List<HpcDataObject> getDataObjects(Object authenticatedToken,
    ...
    try {
      ...
      throw new HpcException("Failed to get data objects: " + e.getMessage(),
          HpcErrorType.DATA_MANAGEMENT_ERROR, null, e);
    }
  }
  ...
  public List<HpcMetadataEntry> getDataObjectMetadata(Object authenticatedToken, String path)
    ...
    try {
      ...
      throw new HpcException(
          "Failed to get metadata of a data object: " + path + ". " + e.getMessage(),
          HpcErrorType.DATA_MANAGEMENT_ERROR, null, e);
    }
  }
  ...
  @Override
  public void updateUser(Object authenticatedToken, String username, String firstName,
      String lastName, HpcUserRole userRole) throws HpcException {
    // Get the iRODS user
  ...
  public List<HpcSubjectPermission> getCollectionPermissions(Object authenticatedToken, String path)
      throws HpcException {
    try {
      return toHpcSubjectPermissions(authenticatedToken, irodsConnection
          .getCollectionAO(authenticatedToken).listPermissionsForCollection(getAbsolutePath(path)));

    } catch (Exception e) {
      throw new HpcException("Failed to get collection permissions: " + e.getMessage(),
          HpcErrorType.DATA_MANAGEMENT_ERROR, null, e);
    }
  }

  @Override
  public HpcSubjectPermission getCollectionPermission(Object authenticatedToken, String path,
      String userId) throws HpcException {
    try {
      return toHpcSubjectPermission(authenticatedToken,
          irodsConnection.getCollectionAO(authenticatedToken)
              .getPermissionForUserName(getAbsolutePath(path), userId),
          false);

    } catch (Exception e) {
      throw new HpcException("Failed to get collection permission for user: " + e.getMessage(),
          HpcErrorType.DATA_MANAGEMENT_ERROR, null, e);
    }
  }

  @Override
  public HpcSubjectPermission acquireCollectionPermission(Object authenticatedToken, String path,
      String userId) throws HpcException {
    try {
  ...
  public void setCollectionPermission(Object authenticatedToken, String path,
      HpcSubjectPermission permissionRequest) throws HpcException {
    ...
    try {
      irodsConnection.getCollectionAO(authenticatedToken).setAccessPermission(
          irodsConnection.getZone(), getAbsolutePath(path), permissionRequest.getSubject(), true,
          permission);
      ...
      throw new HpcException("Failed to set collection permission: " + e.getMessage(),
          HpcErrorType.DATA_MANAGEMENT_ERROR, HpcIntegratedSystem.IRODS, e);
    }
  }
  ...
  public List<HpcSubjectPermission> getDataObjectPermissions(Object authenticatedToken, String path)
    ...
    try {
      return toHpcSubjectPermissions(authenticatedToken, irodsConnection
          .getDataObjectAO(authenticatedToken).listPermissionsForDataObject(getAbsolutePath(path)));

    } catch (Exception e) {
      throw new HpcException("Failed to get data object permissions: " + e.getMessage(),
          HpcErrorType.DATA_MANAGEMENT_ERROR, null, e);
    }
  }

  @Override
  public HpcSubjectPermission getDataObjectPermission(Object authenticatedToken, String path,
      String userId) throws HpcException {
    try {
      return toHpcSubjectPermission(authenticatedToken,
          irodsConnection.getDataObjectAO(authenticatedToken)
      ...
      throw new HpcException("Failed to get data object permission for user: " + e.getMessage(),
          HpcErrorType.DATA_MANAGEMENT_ERROR, null, e);
    }
  }
  ...
  @Override
  public HpcSubjectPermission acquireDataObjectPermission(Object authenticatedToken, String path,
      String userId) throws HpcException {
    try {
      HpcSubjectPermission hsPerm = null;
  ...
  public void setDataObjectPermission(Object authenticatedToken, String path,
    ...
    try {
      irodsConnection.getDataObjectAO(authenticatedToken).setAccessPermission(
          irodsConnection.getZone(), getAbsolutePath(path), permissionRequest.getSubject(),
          permission);
      ...
      throw new HpcException("Failed to set data object permission: " + e.getMessage(),
          HpcErrorType.DATA_MANAGEMENT_ERROR, HpcIntegratedSystem.IRODS, e);
    }
  }
  ...
  public List<String> getGroupMembers(Object authenticatedToken, String groupName)
    ...
    try {
      return toHpcUserIds(
          irodsConnection.getUserGroupAO(authenticatedToken).listUserGroupMembers(groupName));
  ...
  public String getRelativePath(String absolutePath) {
    ...
    String relativePath =
        getAbsolutePath(absolutePath).substring((irodsConnection.getBasePath()).length());
  ...
  /**
   * Create directories. This Jargon API throws runtime exception if the path is invalid, so we
   * catch it and convert to HpcException
   *
  ...
  private List<AVUQueryElement> toIRODSQuery(List<HpcMetadataQuery> metadataQueries)
    ...
    try {
      ...
      for (HpcMetadataQuery metadataQuery : metadataQueries) {
        ...
        queryElements.add(AVUQueryElement.instanceForValueQuery(AVUQueryPart.VALUE, operator,
  ...
  private HpcCollection toHpcCollection(Collection irodsCollection,
    ...
    hpcCollection
        .setCollectionParentName(getRelativePath(irodsCollection.getCollectionParentName()));
  ...
  private IRODSFile getParentPath(Object authenticatedToken, String path) throws HpcException {
    ...
    try {
      ...
      throw new HpcException("Failed to get a parent path" + e.getMessage(),
          HpcErrorType.DATA_MANAGEMENT_ERROR, HpcIntegratedSystem.IRODS, e);
    }
  }
  ...
  private List<HpcSubjectPermission> toHpcSubjectPermissions(Object authenticatedToken,
    ...
    for (UserFilePermission irodsUserPermission : irodsUserPermissions) {
      HpcSubjectPermission hpcSubjectPermission =
          toHpcSubjectPermission(authenticatedToken, irodsUserPermission, true);
   ...
   */
  private FilePermissionEnum toIRODSFilePermissionEnum(HpcPermission permission)
      throws HpcException {
    switch (permission) {
      case OWN:
