...
public class HpcDataTransferProxyImpl implements HpcDataTransferProxy {
	...
	public HpcDataObjectDownloadResponse downloadDataObject(Object authenticatedToken,
		...
		GetObjectRequest request = new GetObjectRequest(downloadRequest.getArchiveLocation().getFileContainerId(),
		...
		try {
			s3Download = s3Connection.getTransferManager(authenticatedToken).download(request,
					downloadRequest.getDestinationFile());
	...
	public HpcPathAttributes getPathAttributes(Object authenticatedToken, HpcFileLocation fileLocation, boolean getSize)
		...
		try {
			boolean objectExists = s3Connection.getTransferManager(authenticatedToken).getAmazonS3Client()
			...
			if (getSize && objectExists) {
				...
				request = new GetObjectMetadataRequest(fileLocation.getFileContainerId(), fileLocation.getFileId());
				pathAttributes.setSize(s3Connection.getTransferManager(authenticatedToken).getAmazonS3Client()
						.getObjectMetadata(request).getContentLength());
			}

		} catch (AmazonServiceException ase) {
			throw new HpcException("[S3] Failed to get metadata: " + request, HpcErrorType.DATA_TRANSFER_ERROR,
					HpcIntegratedSystem.CLEVERSAFE, ase);
	 ...
	 * @param uploadRequestURLExpiration
	 *            The URL expiration (in hours).
