    		                                           ...
    		                                           throws HpcException
   {
    	...
    	if(uploadRequest.getGenerateUploadRequestURL()) {
    	   ...
    	   return generateUploadRequestURL(authenticatedToken, archiveDestinationLocation, 
    			                           objectMetadata, uploadRequest.getUploadRequestURLExpiration());
    	   
    	} else {
    		    // Upload a file 
    		    return uploadDataObject(authenticatedToken, uploadRequest.getSourceFile(),
                                        archiveDestinationLocation, objectMetadata,
    ...
    @Override
    public HpcPathAttributes getPathAttributes(Object authenticatedToken, HpcFileLocation fileLocation,
                                               boolean getSize) 
                                              throws HpcException
    {
    	...
    	try {
    	     boolean objectExists = s3Connection.getTransferManager(authenticatedToken).getAmazonS3Client().
    		                          doesObjectExist(fileLocation.getFileContainerId(), 
    		                                          fileLocation.getFileId());
	    	 ...
	    	 if(getSize && objectExists) {
		        ...
		        throw new HpcException("[S3] Failed to get metadata: " + request, 
	                                   HpcErrorType.DATA_TRANSFER_ERROR, HpcIntegratedSystem.CLEVERSAFE, ase);
                                                        ...
                                                        throws HpcException
	{
		...
		PutObjectRequest request = 
		   new PutObjectRequest(archiveDestinationLocation.getFileContainerId(), 
		                        ...
		                        sourceFile).withMetadata(objectMetadata); 
