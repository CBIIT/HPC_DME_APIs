...
public class HpcNotificationFormatter {
  ...
  public String formatText(
    ...
    if (format == null) {
      throw new HpcException(
          "Notification format not found for: " + notificationType, HpcErrorType.UNEXPECTED_ERROR);
    }
  ...
  private String format(
      String format,
      List<HpcNotificationFormatArgument> formatArguments,
  ...
  private HpcNotificationFormatArgument notificationFormatArgumentFromJSON(
      JSONObject jsonNotificationFormatArgument) throws HpcException {
    ...
    notificationFormatArgument.setIndex(
        Integer.valueOf((String) jsonNotificationFormatArgument.get("index")));
    notificationFormatArgument.setPayloadEntryAttribute(
        (String) jsonNotificationFormatArgument.get("payloadEntryAttribute"));
  ...
  private List<HpcNotificationFormatArgument> notificationFormatArgumentsFromJSON(
    ...
    if (jsonNotificationFormatArguments != null && jsonNotificationFormatArguments.size() > 0) {
      ...
      if (maxIndex + 1 != arguments.size()) {
        throw new HpcException(
            "Invalid arguments index: " + jsonNotificationFormatArguments,
            HpcErrorType.SPRING_CONFIGURATION_ERROR);
      }
    }
  ...
  private HpcNotificationFormat notificationFormatFromJSON(JSONObject jsonNotificationFormat)
    ...
    if (!jsonNotificationFormat.containsKey("subjectFormat")
      ...
      throw new HpcException(
          "Invalid notification format JSON object: " + jsonNotificationFormat,
          HpcErrorType.SPRING_CONFIGURATION_ERROR);
    }
  ...
  private void initNotificationFormats(String notificationFormatPath) throws HpcException {
    ...
    try (FileReader reader = new FileReader(notificationFormatPath)) {
      jsonNotificationFormats =
          (JSONArray) ((JSONObject) new JSONParser().parse(reader)).get("notificationFormats");

    } catch (Exception e) {
      throw new HpcException(
          "Could not open or parse: " + notificationFormatPath,
          HpcErrorType.SPRING_CONFIGURATION_ERROR,
          e);
    }
    ...
    if (jsonNotificationFormats == null || jsonNotificationFormats.isEmpty()) {
      throw new HpcException(
          "No notification formats found in JSON file", HpcErrorType.SPRING_CONFIGURATION_ERROR);
    }
    ...
    while (jsonNotificationFormatIterator.hasNext()) {
      ...
      String systemAdminNotificationTypeStr =
          (String) jsonNotificationFormat.get("systemAdminNotificationType");
      if (StringUtils.isEmpty(eventTypeStr)
          && StringUtils.isEmpty(systemAdminNotificationTypeStr)) {
        throw new HpcException(
            "Invalid event type / system admin notification type: " + jsonNotificationFormat,
            HpcErrorType.SPRING_CONFIGURATION_ERROR);
      }
   ...
   */
  private String getPayloadEntryValue(
      List<HpcEventPayloadEntry> eventPayloadEntries, String attribute) {
