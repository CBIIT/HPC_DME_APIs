...
public class HpcMetadataServiceImpl implements HpcMetadataService {
	...
	public HpcSystemGeneratedMetadata addSystemGeneratedMetadataToDataObject(String path,
			...
			throws HpcException {
		// Input validation.
		if (path == null || dataTransferStatus == null || dataTransferType == null || dataTransferStarted == null) {
			throw new HpcException(INVALID_PATH_METADATA_MSG, HpcErrorType.INVALID_REQUEST_INPUT);
		}
	 ...
	 * @throws HpcException
	 *             if the service invoker is unknown.
	 */
	 ...
	 *
	 * @param existingMetadataEntries
	 *            Existing collection metadata entries.
	 * @param metadataEntries
	 *            Updated collection metadata entries.
	 * @throws HpcException
	 *             If the user tries to update the collection type metadata.
	 */
		...
		for (HpcMetadataEntry metadataEntry : metadataEntries) {
			if (metadataEntry.getAttribute().equals(HpcMetadataValidator.COLLECTION_TYPE_ATTRIBUTE)) {
				if (!metadataEntry.getValue().equals(collectionType)) {
					throw new HpcException("Collection type can't updated", HpcErrorType.INVALID_REQUEST_INPUT);
				}
				break;
			}
		}
