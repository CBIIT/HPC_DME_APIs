...
public class HpcEventServiceImpl implements HpcEventService {
	...
	public static final String COLLECTION_UPDATE_DATA_OBJECT_NAME_PAYLOAD_ATTRIBUTE = "DATA_OBJECT_NAME";
	public static final String DOC_PAYLOAD_ATTRIBUTE = "DOC";
	...
	public static final String DESTINATION_TYPE_PAYLOAD_ATTRIBUTE = "DESTINATION_TYPE";
	...
	public static final String COLLECTION_REGISTRATION_DESCRIPTION_PAYLOAD_VALUE = "Sub collection registered";
	...
	// An indicator whether a collection update notification should be sent to the
	// invoker.
	// By default the invoker is not notified for changes they initiated, but this
	// is handy for
	// testing.
	...
	public void addDataTransferDownloadCompletedEvent(String userId, String path, HpcDownloadTaskType downloadTaskType,
		...
		addDataTransferEvent(userId, HpcEventType.DATA_TRANSFER_DOWNLOAD_COMPLETED, downloadTaskType, downloadTaskId,
				path, null, dataTransferCompleted, null, destinationLocation, null, null, null, destinationType);
	}
	...
	public void addDataTransferDownloadFailedEvent(String userId, String path, HpcDownloadTaskType downloadTaskType,
		...
		addDataTransferEvent(userId, HpcEventType.DATA_TRANSFER_DOWNLOAD_FAILED, downloadTaskType, downloadTaskId, path,
				null, dataTransferCompleted, null, destinationLocation, errorMessage, null, downloadTaskResult, destinationType);
	}
	...
	public void addDataTransferUploadArchivedEvent(String userId, String path, HpcFileLocation sourceLocation,
		...
		addDataTransferEvent(userId, HpcEventType.DATA_TRANSFER_UPLOAD_ARCHIVED, null, null, path, null,
				dataTransferCompleted, sourceLocation, null, null, null, null, null);
		...
		addCollectionUpdatedEvent(parentCollection, DATA_OBJECT_REGISTRATION_PAYLOAD_VALUE,
				DATA_OBJECT_REGISTRATION_DESCRIPTION_PAYLOAD_VALUE, userId, presignURL, size, path, doc);
	}
	...
	public void addDataTransferUploadFailedEvent(String userId, String path, HpcFileLocation sourceLocation,
		...
		addDataTransferEvent(userId, HpcEventType.DATA_TRANSFER_UPLOAD_FAILED, null, null, path, null,
				dataTransferCompleted, sourceLocation, null, errorMessage, null, null, null);
	}
	...
	public void addBulkDataObjectRegistrationCompletedEvent(String userId, String registrationTaskId,
		...
		addDataTransferEvent(userId, HpcEventType.BULK_DATA_OBJECT_REGISTRATION_COMPLETED, null, null, null,
				registrationTaskId, completed, null, null, null, registrationItems, null, null);
	}
	...
	public void addBulkDataObjectRegistrationFailedEvent(String userId, String registrationTaskId, Calendar completed,
		...
		addDataTransferEvent(userId, HpcEventType.BULK_DATA_OBJECT_REGISTRATION_FAILED, null, null, null,
				registrationTaskId, completed, null, null, errorMessage, null, null, null);
	}
	...
	public void generateReportsEvents(List<String> userIds, HpcReportCriteria criteria) throws HpcException {
		...
		if (report.getDoc() != null)
			event.getPayloadEntries().add(toPayloadEntry(HpcReportEntryAttribute.DOC.name(), report.getDoc()));
		...
		if (report.getType() != null)
			event.getPayloadEntries().add(toPayloadEntry(HpcReportEntryAttribute.TYPE.name(), report.getType().name()));
		if (report.getUser() != null)
			event.getPayloadEntries().add(toPayloadEntry(HpcReportEntryAttribute.USER_ID.name(), report.getUser()));
	...
	public void addCollectionUpdatedEvent(String path, String userId) throws HpcException {
		addCollectionUpdatedEvent(path, COLLECTION_METADATA_UPDATE_PAYLOAD_VALUE,
				COLLECTION_METADATA_UPDATE_DESCRIPTION_PAYLOAD_VALUE, userId, null, null, null, null);
	}
	...
	public void addCollectionRegistrationEvent(String path, String userId) throws HpcException {
		addCollectionUpdatedEvent(path, COLLECTION_REGISTRATION_PAYLOAD_VALUE,
				COLLECTION_REGISTRATION_DESCRIPTION_PAYLOAD_VALUE, userId, null, null, null, null);
	}

	@Override
	public void addDataObjectRegistrationEvent(String path, String userId, String presignURL, String size,
		...
		addCollectionUpdatedEvent(path, DATA_OBJECT_REGISTRATION_PAYLOAD_VALUE,
				DATA_OBJECT_REGISTRATION_DESCRIPTION_PAYLOAD_VALUE, userId, presignURL, size, dataObjectPath, doc);
	}
	...
	private void addEvent(HpcEvent event) throws HpcException {
		// Input validation.
		if (event == null || event.getUserIds() == null || event.getUserIds().isEmpty() || event.getType() == null) {
	 ...
	 * @param destinationLocation   (Optional) The data transfer destination
	 *                              location.
	...
	private void addDataTransferEvent(String userId, HpcEventType eventType, HpcDownloadTaskType downloadTaskType,
			...
			List<HpcBulkDataObjectRegistrationItem> registrationItems, HpcDownloadResult downloadTaskResult, HpcDataTransferType destinationType)
			throws HpcException {
		// Input Validation.
		...
		if (downloadTaskId != null) {
			event.getPayloadEntries().add(toPayloadEntry(DOWNLOAD_TASK_ID_PAYLOAD_ATTRIBUTE, downloadTaskId));
		}
		...
		if (path != null) {
			event.getPayloadEntries()
					.add(toPayloadEntry(DATA_OBJECT_PATH_PAYLOAD_ATTRIBUTE, dataManagementProxy.getRelativePath(path)));
		}
		if (registrationTaskId != null) {
			event.getPayloadEntries().add(toPayloadEntry(REGISTRATION_TASK_ID_PAYLOAD_ATTRIBUTE, registrationTaskId));
		}
		...
		if (sourceLocation != null) {
			event.getPayloadEntries().add(toPayloadEntry(SOURCE_LOCATION_PAYLOAD_ATTRIBUTE, toString(sourceLocation)));
		}
		if (destinationLocation != null) {
			event.getPayloadEntries()
					.add(toPayloadEntry(DESTINATION_LOCATION_PAYLOAD_ATTRIBUTE, toString(destinationLocation)));
		}
		...
		if (destinationType != null) {
			event.getPayloadEntries()
					.add(toPayloadEntry(DESTINATION_TYPE_PAYLOAD_ATTRIBUTE, destinationType.toString()));
		}
	 ...
	 * @param updatePayloadValue            The value to set on
	 *                                      COLLECTION_UPDATE_PAYLOAD_ATTRIBUTE
	 *                                      event payload.
	 * @param updateDescriptionPayloadValue The value to set on
	 *                                      COLLECTION_UPDATE_DESCRIPTION_PAYLOAD_ATTRIBUTE
	 *                                      event payload.
	 * @param userId                        The user ID who initiated the action
	 *                                      resulted in collection update event.
	 * @param presignURL                    (Optional) The presigned download URL.
	 * @param size                          (Optional) The data size.
	 * @param doc                           (Optional) The DOC.
	 * @throws HpcException on service failure.
	 */
	private void addCollectionUpdatedEvent(String path, String updatePayloadValue, String updateDescriptionPayloadValue,
			String userId, String presignURL, String size, String dataObjectPath, String doc) throws HpcException {
		// Input Validation.
		...
		if (size != null) {
			event.getPayloadEntries().add(toPayloadEntry(COLLECTION_UPDATE_DATA_OBJECT_SIZE_PAYLOAD_ATTRIBUTE, size));
		}
		if (dataObjectPath != null) {
			event.getPayloadEntries().add(toPayloadEntry(DATA_OBJECT_PATH_PAYLOAD_ATTRIBUTE, dataObjectPath));
			event.getPayloadEntries().add(toPayloadEntry(COLLECTION_UPDATE_DATA_OBJECT_NAME_PAYLOAD_ATTRIBUTE,
					dataObjectPath.substring(dataObjectPath.lastIndexOf("/") + 1)));
		}
		if (doc != null) {
			event.getPayloadEntries().add(toPayloadEntry(DOC_PAYLOAD_ATTRIBUTE, doc));
		}
	 ...
	 * @param userId                        The user ID who initiated the action
	 *                                      resulted in collection update event.
	 * @return A list of user Ids subscribed to this event. This includes users
	 *         registered to the collection path itself and anywhere in the
	...
	private HashSet<String> getCollectionUpdatedEventSubscribedUsers(String path, String updatePayloadValue,
		...
		if (!path.equals("/") && parentCollectionIndex >= 0) {
			userIds.addAll(getCollectionUpdatedEventSubscribedUsers(
					parentCollectionIndex > 0 ? path.substring(0, parentCollectionIndex) : "/", updatePayloadValue,
					updateDescriptionPayloadValue, userId));
		}

		// If configured to - Exclude the user triggered the collection update event
		// from the list.
