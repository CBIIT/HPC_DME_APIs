...
/**
 * A data transfer listener for async streaming uploads. The streaming is done from AWS or Google
 * Drive to S3 archive.
 *
...
public class HpcStreamingUpload implements HpcDataTransferProgressListener {
  ...
  public void transferCompleted(Long bytesTransferred) {
    logger.info("AWS S3 / Google Drive upload completed for: {}", path);
  }

  @Override
  public void transferFailed(String message) {
    logger.error("Streaming (AWS / Google Drive) upload failed for: {} - {}", path, message);
    try {
      eventService.addDataTransferUploadFailedEvent(userId, path, sourceLocation,
          Calendar.getInstance(), message);
    } catch (HpcException e) {
      logger.error("Failed to send upload failed event for AWS S3 / Google Drive streaming", e);
    }
  }
}
