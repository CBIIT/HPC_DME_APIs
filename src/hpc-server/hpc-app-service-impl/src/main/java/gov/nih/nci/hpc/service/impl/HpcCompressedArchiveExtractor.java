...
public class HpcCompressedArchiveExtractor {
  ...
  /**
   * Extract files from compressed archive file into another compressed archive file.
   *
   * @param fromCompressedArchiveFile The compressed archive file to extract files from.
   * @param toCompressedArchiveFile The compressed archive file to extract files to.
  ...
  public int extract(File fromCompressedArchiveFile, File toCompressedArchiveFile,
      HpcCompressedArchiveType compressedArchiveType, List<String> includePatterns,
    ...
    try (FileInputStream fileInputStream = new FileInputStream(fromCompressedArchiveFile);
      ...
      switch (compressedArchiveType) {
        ...
        case ZIP:
          try (
              ArchiveInputStream archiveInputStream =
                  new ZipArchiveInputStream(bufferedInputStream);
              ZipArchiveOutputStream archiveOutputStream =
                  new ZipArchiveOutputStream(bufferedOutputStream)) {
            archiveOutputStream.setUseZip64(Zip64Mode.Always);
            return extract(archiveInputStream, archiveOutputStream, compressedArchiveType,
                compiledIncludePatterns);
          }
        default:
   ...
   * @throws HpcException on service failure.
   */
  private int extract(ArchiveInputStream fromArchiveInputStream,
      ArchiveOutputStream toArchiveOutPutStream, HpcCompressedArchiveType compressedArchiveType,
      List<Pattern> compiledIncludePatterns) throws HpcException {
    int counter = 0;
    ...
    try {
      while ((fromArchiveEntry = fromArchiveInputStream.getNextEntry()) != null) {
        ...
        if (!fromArchiveEntry.isDirectory()
          ...
          toArchiveOutPutStream
              .putArchiveEntry(createArchiveEntry(fromArchiveEntry, compressedArchiveType));
