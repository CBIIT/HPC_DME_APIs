...
/**
 * A data transfer listener for async streaming downloads. The streaming is done from the S3 archive
 * to AWS or Google Drive.
 *
...
public class HpcStreamingDownload implements HpcDataTransferProgressListener {
  ...
  /**
   * Constructs a streaming download object (to keep track of async processing). This constructor is
   * used when a streaming download is restarted, i.e. there is an existing task, and the transfer
  ...
  private void updateDownloadTask(HpcDataObjectDownloadTask downloadTask) throws HpcException {
    ...
    this.downloadTask
        .setGoogleDriveDownloadDestination(downloadTask.getGoogleDriveDownloadDestination());
