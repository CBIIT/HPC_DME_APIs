...
public class HpcDataTransferServiceImpl implements HpcDataTransferService {
	...
	public HpcDataObjectUploadResponse uploadDataObject(HpcGlobusUploadSource globusUploadSource,
			...
			String uploadRequestURLChecksum, String path, String userId, String callerObjectId, String configurationId)
			throws HpcException {
		// Input Validation. One and only one of the first 4 parameters is expected to
		// be provided.

		// Validate an upload source was provided.
		if (globusUploadSource == null && s3UploadSource == null && sourceFile == null && !generateUploadRequestURL) {
		...
		if (s3UploadSource != null) {
			...
			if (!StringUtils.isEmpty(s3UploadSource.getSourceURL()) && s3UploadSource.getSourceSize() == null) {
				throw new HpcException("source URL provided without source size", HpcErrorType.INVALID_REQUEST_INPUT);
			}
		}
		...
		if (generateUploadRequestURL && (globusUploadSource != null || s3UploadSource != null || sourceFile != null)) {
			throw new HpcException(MULTIPLE_UPLOAD_SOURCE_ERROR_MESSAGE, HpcErrorType.INVALID_REQUEST_INPUT);
		}

		// Validate source location exists and accessible.
		Long sourceSize = validateUploadSourceFileLocation(globusUploadSource, s3UploadSource, sourceFile,
				configurationId);
	...
	public HpcDataObjectDownloadResponse downloadDataObject(String path, HpcFileLocation archiveLocation,
			...
			HpcDataTransferType dataTransferType, String configurationId, String userId, boolean completionEvent,
			long size) throws HpcException {
		// Input Validation.
		...
		validateDownloadDestination(globusDownloadDestination, s3DownloadDestination, configurationId, false);
		...
		HpcArchive baseArchiveDestination = dataManagementConfigurationLocator
				.getDataTransferConfiguration(configurationId, dataTransferType).getBaseArchiveDestination();
	...
	public String addSystemGeneratedMetadataToDataObject(HpcFileLocation fileLocation,
		...
		return dataTransferProxies.get(dataTransferType).copyDataObject(
				!globusSyncUpload ? getAuthenticatedToken(dataTransferType, configurationId) : null, fileLocation,
				fileLocation, dataManagementConfigurationLocator
						.getDataTransferConfiguration(configurationId, dataTransferType).getBaseArchiveDestination(),
	...
	public List<HpcDirectoryScanItem> scanDirectory(HpcDataTransferType dataTransferType, HpcS3Account s3Account,
		...
		Object authenticatedToken = s3Account != null
	...
	@Override
	public File getArchiveFile(String configurationId, HpcDataTransferType dataTransferType, String fileId)
			throws HpcException {
		// Input validation.
		if (fileId == null) {
	...
	public void completeDataObjectDownloadTask(HpcDataObjectDownloadTask downloadTask, boolean result, String message,
			Calendar completed, long bytesTransferred) throws HpcException {
		// Input validation
	...
	public void continueDataObjectDownloadTask(HpcDataObjectDownloadTask downloadTask) throws HpcException {
		...
		try {
			...
			completeDataObjectDownloadTask(downloadTask, false, e.getMessage(), Calendar.getInstance(), 0);
			return;
		}
	...
	public void updateDataObjectDownloadTask(HpcDataObjectDownloadTask downloadTask, long bytesTransferred)
		...
		if (dataManagementConfigurationLocator
				.getDataTransferConfiguration(downloadTask.getConfigurationId(), downloadTask.getDataTransferType())
	...
	public HpcCollectionDownloadTask downloadCollection(String path,
		...
		validateDownloadDestination(globusDownloadDestination, s3DownloadDestination, configurationId, true);
	...
	private Object getAuthenticatedToken(HpcDataTransferType dataTransferType, String configurationId)
		...
		Object token = dataTransferProxies.get(dataTransferType).authenticate(dataTransferSystemAccount,
				dataManagementConfigurationLocator.getDataTransferConfiguration(configurationId, dataTransferType)
						.getUrl());
		if (token == null) {
	...
	private HpcDataObjectUploadResponse uploadDataObject(HpcDataObjectUploadRequest uploadRequest,
		...
		HpcDataTransferProgressListener progressListener = uploadRequest.getS3UploadSource() != null
				? new HpcS3Upload(uploadRequest.getPath(), uploadRequest.getUserId(),
		...
		return dataTransferProxies.get(dataTransferType).uploadDataObject(
				!globusSyncUpload ? getAuthenticatedToken(dataTransferType, configurationId) : null, uploadRequest,
				dataTransferConfiguration.getBaseArchiveDestination(),
	...
	private Long validateUploadSourceFileLocation(HpcGlobusUploadSource globusUploadSource,
			HpcS3UploadSource s3UploadSource, File sourceFile, String configurationId) throws HpcException {
		if (sourceFile != null) {
	 ...
	 * @param bulkDownload              True if this is a request to download a list
	 *                                  of files or a collection, or false if this
	 *                                  is a request to download a single data
	 *                                  object.
	...
	private void performS3AsynchronousDownload(HpcDataObjectDownloadRequest downloadRequest,
		...
		try {
			...
			completeDataObjectDownloadTask(s3Download.getDownloadTask(), false, e.getMessage(), Calendar.getInstance(),
					0);
	...
	private class HpcSecondHopDownload implements HpcDataTransferProgressListener {
		...
		public HpcSecondHopDownload(HpcDataObjectDownloadTask downloadTask) throws HpcException {
			...
			// Get the data transfer configuration.
			HpcDataTransferConfiguration dataTransferConfiguration = dataManagementConfigurationLocator
					.getDataTransferConfiguration(downloadTask.getConfigurationId(), HpcDataTransferType.GLOBUS);
		 ...
		 */
		private HpcFileLocation getDownloadSourceLocation(String configurationId, HpcDataTransferType dataTransferType)
				throws HpcException {
			// Get the data transfer configuration.
	...
	private class HpcS3Download implements HpcDataTransferProgressListener {
		...
		/**
		 * Constructs a S3 download object (to keep track of async processing). This
		 * constructor is used when a S3 download is restarted, i.e. there is an
		 * existing task, and the transfer needs to be restarted.
		 *
