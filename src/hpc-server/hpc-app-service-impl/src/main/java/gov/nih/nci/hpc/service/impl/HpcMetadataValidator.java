...
/**
 * Validates various metadata provided by the user. Validation rules are DOC
 * specific.
 *
...
public class HpcMetadataValidator {
	...
	public static final String SOURCE_FILE_NIH_OWNER_ATTRIBUTE = "source_file_nih_owner";
	public static final String SOURCE_FILE_OWNER_ATTRIBUTE = "source_file_owner";
	...
	public static final String REGISTRATION_EVENT_REQUIRED_ATTRIBUTE = "registration_event_required";
	public static final String METADATA_VECTOR_ADDED_ATTRIBUTE = "metadata_vector_added";
	public static final String LINK_SOURCE_PATH_ATTRIBUTE = "link_source_path";
	public static final String LINK_CREATED_ATTRIBUTE = "link_created";
	public static final String COLLECTION_CREATED_ATTRIBUTE = "collection_created";
	public static final String EXTRACTED_METADATA_ATTRIBUTES_ATTRIBUTE = "extracted_metadata_attributes";
	...
	private HpcMetadataValidator() {
		List<String> attributes = Arrays.asList(ID_ATTRIBUTE, REGISTRAR_ID_ATTRIBUTE, REGISTRAR_NAME_ATTRIBUTE,
				CONFIGURATION_ID_ATTRIBUTE, S3_ARCHIVE_CONFIGURATION_ID_ATTRIBUTE,
				...
				DATA_TRANSFER_REQUEST_ID_ATTRIBUTE, DATA_TRANSFER_STATUS_ATTRIBUTE, DATA_TRANSFER_METHOD_ATTRIBUTE,
				DATA_TRANSFER_TYPE_ATTRIBUTE, DATA_TRANSFER_STARTED_ATTRIBUTE, DATA_TRANSFER_COMPLETED_ATTRIBUTE,
				SOURCE_FILE_SIZE_ATTRIBUTE, CALLER_OBJECT_ID_ATTRIBUTE, CHECKSUM_ATTRIBUTE, METADATA_UPDATED_ATTRIBUTE,
				REGISTRATION_EVENT_REQUIRED_ATTRIBUTE, METADATA_VECTOR_ADDED_ATTRIBUTE, LINK_SOURCE_PATH_ATTRIBUTE, LINK_CREATED_ATTRIBUTE, COLLECTION_CREATED_ATTRIBUTE,
				EXTRACTED_METADATA_ATTRIBUTES_ATTRIBUTE, DEEP_ARCHIVE_STATUS_ATTRIBUTE, DEEP_ARCHIVE_DATE_ATTRIBUTE,
				DELETED_DATE_ATTRIBUTE, DME_ID_ATTRIBUTE);
		List<String> collectionAttributes = Arrays.asList(ID_ATTRIBUTE, DME_ID_ATTRIBUTE, REGISTRAR_ID_ATTRIBUTE,
				REGISTRAR_NAME_ATTRIBUTE, CONFIGURATION_ID_ATTRIBUTE, METADATA_UPDATED_ATTRIBUTE, METADATA_VECTOR_ADDED_ATTRIBUTE,
				LINK_SOURCE_PATH_ATTRIBUTE, LINK_CREATED_ATTRIBUTE, COLLECTION_CREATED_ATTRIBUTE);
	...
	/**
	 * Validate collection metadata. Null unit values are converted to empty
	 * strings.
	 *
	 * @param configurationId          Use validation rules of this data management
	 *                                 configuration.
	 * @param existingMetadataEntries  Optional (can be null). The metadata entries
	 *                                 currently associated with the collection or
	 *                                 data object.
	 * @param addUpdateMetadataEntries Optional (can be null) A list of metadata
	 *                                 entries that are being added or updated to
	 *                                 'metadataEntries'.
	...
	public void validateCollectionMetadata(String configurationId, List<HpcMetadataEntry> existingMetadataEntries,
		...
		if (dataManagementConfiguration == null) {
			throw new HpcException("Invalid Configuration: " + configurationId, HpcRequestRejectReason.INVALID_DOC);
		}
	...
	/**
	 * Validate data object metadata. Null unit values are converted to empty
	 * strings.
	 *
	 * @param configurationId          Use validation rules of this data management
	 *                                 configuration.
	 ...
	 * @param collectionType           (Optional) The collection type. In case of
	 *                                 data object, the type of collection
	...
	private void validateMetadata(List<HpcMetadataEntry> existingMetadataEntries,
		...
		// Determining the collection type of this collection or collection containing
		// this data object.
		...
		for (HpcMetadataValidationRule metadataValidationRule : metadataValidationRules) {
			...
			if (skipRule(metadataValidationRule, collectionType)) {
				continue;
			}

			HpcMetadataEntry defaultMetadataEntry = generateDefaultMetadataEntry(metadataValidationRule,
					metadataEntriesMap);
			...
			if (metadataValidationRule.getMandatory()
				...
				if (StringUtils.isEmpty(errors.get(MANDATORY_METADATA_ERROR))) {
					errors.put(MANDATORY_METADATA_ERROR,
							"Missing or empty mandatory metadata: " + metadataValidationRule.getAttribute());
				} else {
					errors.put(MANDATORY_METADATA_ERROR,
			...
			if (metadataValidationRule.getValidValues() != null && !metadataValidationRule.getValidValues().isEmpty()) {
				...
				// is not a mandatory or conditional attribute, then it's value shouldn't
				// matter.
			...
			if (metadataValidationRule.getDateFormat() != null
				...
				try {
					...
					addUpdateMetadataEntries
							.removeIf(entry -> entry.getAttribute().equals(metadataValidationRule.getAttribute()));
					addUpdateMetadataEntries.add(formattedDateEntry);
				} catch (ParseException e) {
					throw new HpcException(
							"Metadata format is invalid: " + metadataValidationRule.getAttribute() + ": " + value
		...
		if (!errors.isEmpty()) {
			String errorMessage = errors.get(MANDATORY_METADATA_ERROR) != null ? errors.get(MANDATORY_METADATA_ERROR)
					: errors.get(CONDITIONAL_METADATA_ERROR);
	 ...
	 * @param collectionType         the collection type for collection metadata, or
	 ...
	 */
	private HpcMetadataEntry generateDefaultMetadataEntry(HpcMetadataValidationRule metadataValidationRule,
			Map<String, String> metadataEntriesMap) {
		// Apply default value/unit if default is defined and metadata was not provided.
		if (metadataValidationRule.getDefaultValue() != null && !metadataValidationRule.getDefaultValue().isEmpty()
