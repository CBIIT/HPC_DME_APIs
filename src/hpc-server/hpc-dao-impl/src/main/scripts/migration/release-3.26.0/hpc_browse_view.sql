...
SELECT coll.coll_id                                                                                                 AS id,
       ...
       sum(coll_size.TOTALSIZE)                                                                                     AS data_size,
       NEW_TIME(to_timestamp('1970-01-01', 'YYYY-MM-DD') + numtodsinterval(coll.modify_ts, 'SECOND'), 'GMT',
                'EST')                                                                                              AS uploaded,
       case when meta_softlink.META_ATTR_VALUE is not null then 1 else 0 end                                        as softlink
FROM r_coll_main coll
    ...
    JOIN r_meta_main meta_softlink ON meta_softlink.meta_id = map_softlink.meta_id AND
         ...
         LEFT JOIN r_report_collection_size coll_size ON coll_size.COLL_NAME = coll.COLL_NAME
group by coll.coll_id, coll.coll_name,
         NEW_TIME(to_timestamp('1970-01-01', 'YYYY-MM-DD') + numtodsinterval(coll.modify_ts, 'SECOND'), 'GMT', 'EST'),
         case when meta_softlink.META_ATTR_VALUE is not null then 1 else 0 end;