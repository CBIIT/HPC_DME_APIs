...
public class HpcReportsDAOImpl implements HpcReportsDAO {
  ...
  // ---------------------------------------------------------------------//
  // File size range
  private static final String FILE_RANGE_SELECT =
      "select (case when to_number(a.meta_attr_value, '9999999999999999999') <=  1000000 then 'range1' "
  ...
  private static final String FILE_RANGE_GROUP =
      " group by (case when to_number(a.meta_attr_value, '9999999999999999999') <= 1000000 then 'range1' "
          + "when to_number(a.meta_attr_value, '9999999999999999999') > 1000000 and to_number(a.meta_attr_value, '9999999999999999999')     <= 10000000 then 'range2' "
          + "when to_number(a.meta_attr_value, '9999999999999999999') > 10000000 and to_number(a.meta_attr_value, '9999999999999999999')    <= 50000000 then 'range3' "
          + "when to_number(a.meta_attr_value, '9999999999999999999') > 50000000 and to_number(a.meta_attr_value, '9999999999999999999')    <= 100000000 then 'range4' "
          + "when to_number(a.meta_attr_value, '9999999999999999999') > 100000000 and to_number(a.meta_attr_value, '9999999999999999999')   <= 500000000 then 'range5' "
  ...
  private static final String TOTAL_NUM_OF_DATA_OBJECTS_SQL =
      "SELECT count(distinct data_id) totalObjs FROM r_report_data_objects";

  private static final String TOTAL_NUM_OF_COLLECTIONS_BY_NAME_SQL =
      "select meta_attr_value attr, count(coll_id) cnt from r_report_collection_type group by meta_attr_value";

  private static final String AVG_NUM_OF_DATA_OBJECT_META_ATTRS_SQL =
      "SELECT count(meta_attr_name) / greatest( count(distinct data_id), 1 ) FROM r_report_data_objects";
  ...
  private static final String TOTAL_NUM_OF_COLLECTIONS_BY_NAME_AND_DATE_SQL =
      "select meta_attr_value attr, count(coll_id) cnt from r_report_collection_type where CAST(create_ts as double precision) BETWEEN ? AND ? group by meta_attr_value";

  private static final String AVG_NUM_OF_DATA_OBJECT_META_ATTRS_BY_DATE_SQL =
      "SELECT count(meta_attr_name) / greatest( count(distinct data_id), 1 ) FROM r_report_data_objects where CAST(create_ts as double precision) BETWEEN ? AND ?";
  ...
  private static final String AVG_NUM_OF_DATA_OBJECT_META_ATTRS_BY_DOC_SQL =
  ...
  private static final String AVG_NUM_OF_DATA_OBJECT_META_ATTRS_BY_DOC_DATE_SQL =
      "SELECT count(a.meta_attr_name) / greatest( count(distinct data_id), 1 ) "
          + "FROM r_report_data_objects a, r_report_registered_by_doc b "
  ...
  private static final String AVG_NUM_OF_DATA_OBJECT_META_ATTRS_BY_USER_SQL =
      "SELECT count(a.meta_attr_name) / greatest( count(distinct data_id), 1 ) "
  ...
  private static final String AVG_NUM_OF_DATA_OBJECT_META_ATTRS_BY_USER_DATE_SQL =
      "SELECT count(a.meta_attr_name) / greatest( count(distinct data_id), 1 ) "
          + "FROM r_report_data_objects a, r_report_registered_by b "
  ...
  private static final String DOCS_SQL =
      "select distinct meta_attr_value from public.r_meta_main where meta_attr_name='registered_by_doc'";
  ...
  private String getTotalDataObjSize(HpcReportCriteria criteria, Long[] dates, String[] docArg,
    ...
    if (criteria.getType().equals(HpcReportType.USAGE_SUMMARY))
      ...
      dataSize =
          jdbcTemplate.queryForObject(TOTAL_NUM_OF_DATA_OBJECTS_BY_DATE_SQL, dates, Long.class);
    else if (criteria.getType().equals(HpcReportType.USAGE_SUMMARY_BY_DOC))
      dataSize =
          jdbcTemplate.queryForObject(TOTAL_NUM_OF_DATA_OBJECTS_BY_DOC_SQL, docArg, Long.class);
    else if (criteria.getType().equals(HpcReportType.USAGE_SUMMARY_BY_DOC_BY_DATE_RANGE))
      dataSize = jdbcTemplate.queryForObject(TOTAL_NUM_OF_DATA_OBJECTS_BY_DOC_DATE_SQL, docDateArgs,
          Long.class);
    else if (criteria.getType().equals(HpcReportType.USAGE_SUMMARY_BY_USER))
      dataSize =
          jdbcTemplate.queryForObject(TOTAL_NUM_OF_DATA_OBJECTS_BY_USER_SQL, userArg, Long.class);
    else if (criteria.getType().equals(HpcReportType.USAGE_SUMMARY_BY_USER_BY_DATE_RANGE))
      ...
      dataSize = jdbcTemplate.queryForObject(TOTAL_NUM_OF_DATA_OBJECTS_BY_PATH_DATE_SQL,
          pathDateArgs, Long.class);
  ...
  private List<Map<String, Object>> getTotalCollectionsSize(HpcReportCriteria criteria,
    ...
    if (criteria.getType().equals(HpcReportType.USAGE_SUMMARY))
      ...
      list =
          jdbcTemplate.queryForList(TOTAL_NUM_OF_COLLECTIONS_BY_NAME_PATH_DATE_SQL, pathDateArgs);
    return list;
  }
  ...
  private String getTotalMetaAttrCount(HpcReportCriteria criteria, Long[] dates, String[] docArg,
    ...
    if (criteria.getType().equals(HpcReportType.USAGE_SUMMARY))
      metaAttrCount =
          jdbcTemplate.queryForObject(AVG_NUM_OF_DATA_OBJECT_META_ATTRS_SQL, Long.class);
    else if (criteria.getType().equals(HpcReportType.USAGE_SUMMARY_BY_DATE_RANGE))
      metaAttrCount = jdbcTemplate.queryForObject(AVG_NUM_OF_DATA_OBJECT_META_ATTRS_BY_DATE_SQL,
          dates, Long.class);
    else if (criteria.getType().equals(HpcReportType.USAGE_SUMMARY_BY_DOC)) {
      String[] newDoc = new String[1];
      newDoc[0] = docArg[0];
      metaAttrCount = jdbcTemplate.queryForObject(AVG_NUM_OF_DATA_OBJECT_META_ATTRS_BY_DOC_SQL,
          newDoc, Long.class);
    } else if (criteria.getType().equals(HpcReportType.USAGE_SUMMARY_BY_DOC_BY_DATE_RANGE)) {
      ...
      metaAttrCount = jdbcTemplate.queryForObject(AVG_NUM_OF_DATA_OBJECT_META_ATTRS_BY_USER_SQL,
          newUser, Long.class);
    } else if (criteria.getType().equals(HpcReportType.USAGE_SUMMARY_BY_USER_BY_DATE_RANGE)) {
  ...
  private List<Map<String, Object>> getFileSizeRange(HpcReportCriteria criteria,
    ...
    if (criteria.getType().equals(HpcReportType.USAGE_SUMMARY)) {
      ...
      return jdbcTemplate.query(FILE_RANGE_SELECT + FILE_RANGE_FROM + FILE_RANGE_GROUP,
          fileSizeRangeRowMapper, null);
    } else if (criteria.getType().equals(HpcReportType.USAGE_SUMMARY_BY_DATE_RANGE)) {
      FILE_RANGE_FROM = " from r_report_source_file_size a ";
  ...
  public static String humanReadableByteCount(double bytes, final boolean useSIUnits) {
	  bytes = 1652442589;
