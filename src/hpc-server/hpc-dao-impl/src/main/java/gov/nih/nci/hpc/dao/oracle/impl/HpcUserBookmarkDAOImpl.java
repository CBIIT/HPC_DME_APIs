...
public class HpcUserBookmarkDAOImpl implements HpcUserBookmarkDAO {
	...
	public void deleteBookmark(String nciUserId, String bookmarkName) throws HpcException {
		try {
			...
			throw new HpcException("Failed to delete a bookmark" + e.getMessage(), HpcErrorType.DATABASE_ERROR,
					HpcIntegratedSystem.ORACLE, e);
		}
	}
	...
	public List<HpcBookmark> getBookmarks(String nciUserId) throws HpcException {
		try {
			return jdbcTemplate.query(GET_USER_BOOKMARKS_SQL, userBookmarkRowMapper, nciUserId);

		} catch (IncorrectResultSizeDataAccessException notFoundEx) {
			return null;

		} catch (DataAccessException e) {
			throw new HpcException("Failed to get user bookmarks: " + e.getMessage(), HpcErrorType.DATABASE_ERROR,
					HpcIntegratedSystem.ORACLE, e);
		}
	}
	...
	public HpcBookmark getBookmark(String nciUserId, String bookmarkName) throws HpcException {
		try {
			return jdbcTemplate.queryForObject(GET_USER_BOOKMARK_SQL, userBookmarkRowMapper, nciUserId, bookmarkName);
			...
			throw new HpcException("Failed to get a user bookamrk: " + e.getMessage(), HpcErrorType.DATABASE_ERROR,
					HpcIntegratedSystem.ORACLE, e);
		}
	}
	...
	public List<HpcBookmark> getBookmarksByPath(String nciUserId, String bookmarkPath) throws HpcException {
		try {
			return jdbcTemplate.query(GET_USER_BOOKMARKS_BY_PATH_SQL, userBookmarkRowMapper, nciUserId, bookmarkPath);

		} catch (DataAccessException e) {
			throw new HpcException("Failed to get bookamrks for user " + nciUserId + " for the given path "
					+ bookmarkPath + ": " + e.getMessage(), HpcErrorType.DATABASE_ERROR, HpcIntegratedSystem.ORACLE, e);
		}
	}
}
