...
public class HpcDataManagementRestServiceImpl extends HpcRestServiceImpl implements HpcDataManagementRestService {
	...
	@Override
	public Response getCollectionChildrenWithPaging(String path, Integer offset, Boolean report) {
		HpcCollectionListDTO collections = new HpcCollectionListDTO();
		try {
			HpcCollectionDTO collection = dataManagementBusService.getCollectionChildrenWithPaging(toNormalizedPath(path), 
					offset != null ? offset : 0, report != null ? report : false);
			if (collection != null) {
				collections.getCollections().add(collection);
			}
	...
	@Override
	public Response downloadCollectionFromExternalSource(String path,
			gov.nih.nci.hpc.dto.datamanagement.v2.HpcDownloadRequestDTO downloadRequest) {
		HpcCollectionDownloadResponseDTO downloadResponse = null;
		try {
			downloadResponse = dataManagementBusService.downloadCollectionFromExternalSource(toNormalizedPath(path), downloadRequest);
	...
	@Override
	public Response getDownloadSummary(Integer page, Boolean totalCount, Integer pageSize) {
