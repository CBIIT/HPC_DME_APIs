...
public class HpcSecurityRestServiceImpl extends HpcRestServiceImpl implements HpcSecurityRestService {
	...
	public Response registerGroup(String groupName, HpcGroupMembersRequestDTO groupMembersRequest) {
		...
		try {
			groupMembersResponse = securityBusService
					.registerGroup(URLDecoder.decode(groupName, StandardCharsets.UTF_8.name()), groupMembersRequest);

		} catch (HpcException | UnsupportedEncodingException e) {
			return errorResponse(e);
		}
	...
	public Response updateGroup(String groupName, HpcGroupMembersRequestDTO groupMembersRequest) {
		...
		try {
			groupMembersResponse = securityBusService
					.updateGroup(URLDecoder.decode(groupName, StandardCharsets.UTF_8.name()), groupMembersRequest);
	...
	public Response getGroup(String groupName) {
		...
		try {
			groupMembers = securityBusService.getGroup(URLDecoder.decode(groupName, StandardCharsets.UTF_8.name()));
	...
	public Response deleteGroup(String groupName) {
		try {
			securityBusService.deleteGroup(URLDecoder.decode(groupName, StandardCharsets.UTF_8.name()));
