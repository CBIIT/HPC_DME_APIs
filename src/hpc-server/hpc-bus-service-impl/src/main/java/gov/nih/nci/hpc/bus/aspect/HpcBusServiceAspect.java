...
/**
 * HPC Bus Services Aspect - implement cross cutting concerns: 1. Basic business
 * service profiler - log execution time. 2. Exception logger - logging when
 * exceptions are thrown by Bus Services API impl. 3. Notify System
 * Administrator if an error occurred with an integrated system (iRODS, LDAP,
...
public class HpcBusServiceAspect {
	 ...
	 * gov.nih.nci.hpc.bus package, and implemented by a concrete class in
	...
	public Object profileService(ProceedingJoinPoint joinPoint) throws Throwable {
		...
		try {
			...
			logger.debug("{} business service completed in {} milliseconds.", businessService, executionTime);
		}
	}
	...
	public void logException(JoinPoint joinPoint, HpcException exception) {
		String businessService = joinPoint.getSignature().toShortString();
		if (!exception.getSuppressStackTraceLogging()) {
			logger.error("{} business service error: {}", businessService, exception.getMessage(), exception);
		} else {
	...
	/**
	 * Advice that alerts a system administrator of a problem with an integrated
	 * system.
	 *
	 * @param joinPoint The join point.
	 * @param exception The exception to log.
