...
public class HpcDataManagementBusServiceImpl implements HpcDataManagementBusService {
	...
	public boolean registerCollection(String path, HpcCollectionRegistrationDTO collectionRegistration, String userId,
		...
		createParentCollections(path, collectionRegistration.getCreateParentCollections(),
				collectionRegistration.getParentCollectionMetadataEntries(), userId, userName, configurationId);
	...
	public HpcCollectionDownloadResponseDTO downloadCollection(String path, HpcDownloadRequestDTO downloadRequest)
		...
		if (downloadRequest.getDestination() == null) {
			throw new HpcException("Null destination in download request", HpcErrorType.INVALID_REQUEST_INPUT);
		}
		...
		HpcCollectionDownloadTask collectionDownloadTask = dataTransferService.downloadCollection(path,
				downloadRequest.getDestination(), securityService.getRequestInvoker().getNciAccount().getUserId(),
	...
	@Override
	public HpcBulkDataObjectDownloadResponseDTO downloadDataObjects(HpcBulkDataObjectDownloadRequestDTO downloadRequest)
			throws HpcException {
		// Input validation.
		...
		HpcCollectionDownloadTask collectionDownloadTask = dataTransferService.downloadDataObjects(dataObjectPathsMap,
				downloadRequest.getDestination(), securityService.getRequestInvoker().getNciAccount().getUserId());
	...
	@Override
	public HpcCollectionDownloadStatusDTO getDataObjectsDownloadStatus(String taskId) throws HpcException {
	...
	public HpcDownloadSummaryDTO getDownloadSummary(int page, boolean totalCount) throws HpcException {
		...
		if (totalCount) {
			...
			downloadSummary.setTotalCount(
					(page == 1 && count < limit) ? count : dataTransferService.getDownloadResultsCount(userId));
		}
	...
	public HpcUserPermsForCollectionsDTO getUserPermissionsOnCollections(String[] collectionPaths, String userId)
		...
		if (dataManagementSecurityService.userExists(userId)) {
			...
			for (String someCollectionPath : collectionPaths) {
				HpcPermissionForCollection permForColl = _fetchCollectionPermission(someCollectionPath, userId);
				if (null != permForColl) {
	...
	private HpcPermissionForCollection _fetchCollectionPermission(String path, String userId) throws HpcException {
		// Input validation.
		if (path == null) {
			throw new HpcException("Null path", HpcErrorType.INVALID_REQUEST_INPUT);
		}
	...
	public HpcDataObjectRegistrationResponseDTO registerDataObject(String path,
		...
		if (StringUtils.isEmpty(path) || dataObjectRegistration == null) {
			throw new HpcException("Null path or dataObjectRegistrationDTO", HpcErrorType.INVALID_REQUEST_INPUT);
		}
		...
		createParentCollections(path, dataObjectRegistration.getCreateParentCollections(),
				dataObjectRegistration.getParentCollectionMetadataEntries(), userId, userName, configurationId);
		...
		boolean generateUploadRequestURL = dataObjectRegistration.getGenerateUploadRequestURL() != null
		...
		if (responseDTO.getRegistered()) {
			...
			try {
				...
				HpcDataObjectUploadResponse uploadResponse = dataTransferService.uploadDataObject(source,
						dataObjectFile, generateUploadRequestURL, path, userId,
			...
			// uploaded yet).
			responseDTO.setUploadRequestURL(updateDataObject(path, dataObjectRegistration.getMetadataEntries(),
					collectionType, generateUploadRequestURL, userId, dataObjectRegistration.getCallerObjectId()));
		}
	...
	public HpcBulkDataObjectRegistrationResponseDTO registerDataObjects(
		...
		for (HpcDataObjectRegistrationItemDTO dataObjectRegistrationItem : bulkDataObjectRegistrationRequest
			...
			dataObjectRegistrationRequest.getParentCollectionMetadataEntries()
	...
	public HpcDataObjectDownloadResponseDTO downloadDataObject(String path, HpcDownloadRequestDTO downloadRequest,
		...
		HpcDataObjectDownloadResponse downloadResponse = dataTransferService.downloadDataObject(path,
				metadata.getArchiveLocation(), downloadRequest.getDestination(), metadata.getDataTransferType(),
	...
	/**
	 * Get the data object source size. (Either source or dataObjectFile are not
	 * null)
	 * 
	 * @param source
	 ...
	 *             on service failure.
	 */
	private Long getSourceSize(HpcFileLocation source, HpcDataTransferType dataTransferType, File dataObjectFile,
			String configurationId) throws HpcException {
		if (source != null) {
			return dataTransferService.getPathAttributes(dataTransferType, source, true, configurationId).getSize();
		}
	 ...
	 * 
	 * @param systemGeneratedMetadata
	 *            The system generated metadata of the data object.
	 * @return The transfer % completion if transfer is in progress, or null
	 *         otherwise. e.g 86%.
	 */
	private String getDataTransferUploadPercentCompletion(HpcSystemGeneratedMetadata systemGeneratedMetadata) {
		...
		long transferSize = 0;
		try {
			transferSize = dataTransferService.getDataTransferSize(systemGeneratedMetadata.getDataTransferType(),
	 ...
	 * 
	 * @param entityPermissionsRequest
	 *            The request to validate.
	 * @throws HpcException
	 *             if found an invalid request in the list.
	 */
	 ...
	 * 
	 * @param path
	 *            The path of the entity trigger this event.
	 * @param collectionRegistered
	 *            An indicator if a collection was registered.
	 * @param dataObjectRegistered
	 *            An indicator if a data object was registered.
	 */
	 ...
	 * @param taskId
	 *            The data object download task ID.
	...
	private HpcDataObjectDownloadResponseDTO toDownloadResponseDTO(HpcFileLocation destinationLocation,
			File destinationFile, String taskId) {
		// Construct and return a DTO
	 ...
	 * 
	 * @param path
	 *            The data object's path.
	 * @param createParentCollections
	 *            The indicator whether to create parent collections.
	 * @param parentCollectionMetadataEntries
	 *            The parent collection metadata entries.
	 * @param userId
	 *            The registrar user-id.
	 * @param userName
	 *            The registrar name.
	 * @param configurationId
	 *            The data management configuration ID.
	...
	private void createParentCollections(String path, Boolean createParentCollections,
			List<HpcMetadataEntry> parentCollectionMetadataEntries, String userId, String userName,
			String configurationId) throws HpcException {
		...
		if (createParentCollections != null && createParentCollections
			...
			if (parentCollectionMetadataEntries == null || parentCollectionMetadataEntries.isEmpty()) {
				throw new HpcException("Null or empty parent metadata entries", HpcErrorType.INVALID_REQUEST_INPUT);
			}
			...
			registerCollection(path.substring(0, path.lastIndexOf('/')), collectionRegistration, userId, userName,
					configurationId);
		}
	}
	 ...
	 * 
	 * @param path
	 *            The entity path (collection or data object).
	 * @param collection
	 *            True if the path is a collection, False if the path is a data
	 *            object.
	 * @param userPermissionRequests
	 *            The list of user permissions requests.
	 ...
	 *            True if the path is a collection, False if the path is a data
	 *            object.
	 * @param groupPermissionRequests
	 *            The list of group permissions requests.
	 * @return A list of responses to the permission requests.
	 */
	 ...
	 * @param taskType
	 *            COLLECTION or DATA_OBJECT_LIST.
	 ...
	 *            The download status to populate the items into.
	 * @param items
	 *            The collection / bulk download items.
	 */
	 ...
	 *            The data object path.
	 * @param systemGeneratedMetadata
	 *            The system generetaed metadata.
	 * @param dataObjectDeleteResponse
	 *            The deletion response DTO.
	 */
	 ...
	 *            The data transfer upload system generetaed metadata.
	 */
	 ...
	 * 
	 * @param file
	 *            The file to validate checksum.
	 * @param checksum
	 *            The checksum value provided by the caller.
	 * @throws HpcException
	 *             If the calculated checksum doesn't match the provided value.
	 */
	 ...
	 * 
	 * @param directoryScanRegistrationItems
	 *            The directory scan registration items.
	...
	private List<HpcDataObjectRegistrationItemDTO> toDataObjectRegistrationItems(
		...
		for (HpcDirectoryScanRegistrationItemDTO directoryScanRegistrationItem : directoryScanRegistrationItems) {
			...
			HpcPathAttributes pathAttributes = dataTransferService.getPathAttributes(HpcDataTransferType.GLOBUS,
					directoryScanRegistrationItem.getScanDirectoryLocation(), false, configurationId);
			if (!pathAttributes.getExists()) {
				throw new HpcException(
						"Endpoint or path doesn't exist: " + directoryScanRegistrationItem.getScanDirectoryLocation(),
						HpcErrorType.INVALID_REQUEST_INPUT);
			}
			if (!pathAttributes.getIsAccessible()) {
				throw new HpcException(
						"Endpoint is not accessible: " + directoryScanRegistrationItem.getScanDirectoryLocation(),
						HpcErrorType.INVALID_REQUEST_INPUT);
			}
			if (!pathAttributes.getIsDirectory()) {
				throw new HpcException(
						"Endpoint is not a directory: " + directoryScanRegistrationItem.getScanDirectoryLocation(),
						HpcErrorType.INVALID_REQUEST_INPUT);
			}
			...
			dataTransferService
					.scanDirectory(HpcDataTransferType.GLOBUS, directoryScanRegistrationItem.getScanDirectoryLocation(),
							configurationId, directoryScanRegistrationItem.getIncludePatterns(),
					...
					.forEach(scanItem -> dataObjectRegistrationItems.add(toDataObjectRegistrationItem(scanItem,
							basePath, directoryScanRegistrationItem.getScanDirectoryLocation().getFileContainerId(),
	 ...
	 *            The scan item.
	 * @param basePath
	 *            The base path to register the scan item with.
	 * @param sourceFileContainerId
	 *            The container ID containing the scan item.
	 ...
	 *            The list of metadata entries to update.
	 * @param collectionType
	 *            The type of collection containing the data object.
	 * @param generateUploadRequestURL
	 *            Indicator whether to re-generate the request upload URL.
	 * @param userId
	 *            The userId updating the data object.
	 * @param callerObjectId
	 *            The caller's object ID.
	 ...
	 *             on service failure.
	 */
	private String updateDataObject(String path, List<HpcMetadataEntry> metadataEntries, String collectionType,
			boolean generateUploadRequestURL, String userId, String callerObjectId) throws HpcException {
		// Get the metadata for this data object.
		...
		if (generateUploadRequestURL) {
			...
			HpcDataObjectUploadResponse uploadResponse = dataTransferService.uploadDataObject(null, null, true, path,
