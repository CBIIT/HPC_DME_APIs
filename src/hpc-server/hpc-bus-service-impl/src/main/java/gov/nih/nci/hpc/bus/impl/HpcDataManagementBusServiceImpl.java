...
public class HpcDataManagementBusServiceImpl implements HpcDataManagementBusService {
		...
		if (responseDTO.getRegistered()) {
			...
			try {
				...
				HpcDataObjectUploadResponse uploadResponse = dataTransferService.uploadDataObject(
						...
						dataObjectFile, generateUploadRequestURL,
				...
				// object.
				HpcSystemGeneratedMetadata systemGeneratedMetadata = metadataService
						.addSystemGeneratedMetadataToDataObject(path, uploadResponse.getArchiveLocation(),
								uploadResponse.getUploadSource(), uploadResponse.getDataTransferRequestId(),
			...
			responseDTO.setUploadRequestURL(updateDataObject(path, dataObjectRegistration.getMetadataEntries(),
					collectionType, generateUploadRequestURL, dataObjectRegistration.getChecksum(), userId,
	...
	public HpcDataObjectDownloadResponseDTO downloadDataObject(String path, HpcDownloadRequestDTO downloadRequest,
		...
		return toDownloadResponseDTO(downloadResponse.getDestinationLocation(), downloadResponse.getDestinationFile(),
				downloadResponse.getDownloadTaskId(), null, metadata.getDataTransferType().value());
	}
	 ...
	 * @param dataTransferType    The data transfer type performing the download (S3
	 *                            or Globus).
	...
	private HpcDataObjectDownloadResponseDTO toDownloadResponseDTO(HpcFileLocation destinationLocation,
			File destinationFile, String taskId, String downloadRequestURL, String dataTransferType) {
		// Construct and return a DTO
	...
	private List<HpcDataObjectRegistrationItemDTO> toDataObjectRegistrationItems(
		...
		for (HpcDirectoryScanRegistrationItemDTO directoryScanRegistrationItem : directoryScanRegistrationItems) {
			...
			if (directoryScanRegistrationItem.getGlobusScanDirectory() != null
					&& directoryScanRegistrationItem.getS3ScanDirectory() != null) {
				throw new HpcException("Both Globus and S3 directory provided", HpcErrorType.INVALID_REQUEST_INPUT);
			}
	...
	private String updateDataObject(String path, List<HpcMetadataEntry> metadataEntries, String collectionType,
			boolean generateUploadRequestURL, String checksum, String userId, String callerObjectId)
			throws HpcException {
		// Get the metadata for this data object.
		...
		if (generateUploadRequestURL) {
			...
			HpcDataObjectUploadResponse uploadResponse = dataTransferService.uploadDataObject(null, null, null, true,
					checksum, path, userId, callerObjectId, systemGeneratedMetadata.getConfigurationId());
