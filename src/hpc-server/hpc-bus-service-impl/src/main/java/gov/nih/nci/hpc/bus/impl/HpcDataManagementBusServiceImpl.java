...
public class HpcDataManagementBusServiceImpl implements HpcDataManagementBusService {
	...
	public HpcDataObjectDownloadResponseDTO downloadDataObjectFromExternalSource(String path, HpcDownloadRequestDTO downloadRequest)
			throws HpcException {
		// Validate that the external archive link directory property is configured, as it is required for saving the temporary archive links for external downloads.
		...
		try {
			dmePath = buildAndValidatePermanentArchiveLinkPath(path, basePath, posixPath);
		...
		// Registration Step
		try {
			// Check if the temporary archive link already exists in DME, if it does not create a registration for it
	...
	private String buildAndValidatePermanentArchiveLinkPath(String path, String basePath, String posixPath) throws HpcException {
