...
public class HpcSystemBusServiceImpl implements HpcSystemBusService {
		...
		for (HpcDataObject dataObject : dataObjectsReceived) {
			...
			try {
				...
				HpcDataObjectUploadResponse uploadResponse = dataTransferService.uploadDataObject(
						toGlobusUploadSource(systemGeneratedMetadata.getSourceLocation()), null, null, false, null,
		...
		for (HpcDataObject dataObject : dataObjectsStreamingStopped) {
			...
			try {
				...
				HpcDataObjectUploadResponse uploadResponse = dataTransferService.uploadDataObject(null,
						...
						systemGeneratedMetadata.getCallerObjectId(), systemGeneratedMetadata.getConfigurationId());
	...
	public void processTemporaryArchive() throws HpcException {
		...
		for (HpcDataObject dataObject : dataObjectsInTemporaryArchive) {
			...
			try {
				...
				HpcDataObjectUploadResponse uploadResponse = dataTransferService.uploadDataObject(null, null, file,
						false, null, path, systemGeneratedMetadata.getRegistrarId(),
						systemGeneratedMetadata.getCallerObjectId(), systemGeneratedMetadata.getConfigurationId());
	 ...
	 * @param userId                    The user ID who requested the collection
	 *                                  download.
	...
	private List<HpcCollectionDownloadTaskItem> downloadDataObjects(List<String> dataObjectPaths,
		...
		for (String dataObjectPath : dataObjectPaths) {
			downloadItems
					.add(downloadDataObject(dataObjectPath, globusDownloadDestination, s3DownloadDestination, userId));
		}
	...
	private HpcCollectionDownloadTaskItem downloadDataObject(String path,
		...
		dataObjectDownloadRequest
				.setGlobusDownloadDestination(calculateGlobusDownloadDestination(globusDownloadDestination, path));
	 ...
	 *                                   calculate the destination location for.
	...
	private void completeCollectionDownloadTask(HpcCollectionDownloadTask downloadTask, boolean result, String message)
		...
		addDataTransferDownloadEvent(downloadTask.getUserId(), path, downloadTask.getType(), downloadTask.getId(),
				downloadTask.getS3DownloadDestination() != null ? HpcDataTransferType.S_3 : HpcDataTransferType.GLOBUS,
	 ...
	 * (succeeded or failed), record the result.
	 *
	 * @param downloadTask The download task to complete.
	 * @throws HpcException on service failure.
	 */
	 ...
	 * @param configurationId The data management configuration ID. This is to get
	...
	private boolean generatedURLExpired(Calendar urlCreated, String configurationId) {
		...
		expiration.setTime(urlCreated.getTimeInMillis()
				+ 1000 * 60 * 60 * dataManagementConfiguration.getS3Configuration().getUploadRequestURLExpiration());
	 ...
	 */
	private HpcS3UploadSource toS3UploadSource(HpcFileLocation sourceLocation, String sourceURL, Long sourceSize) {
