...
public class HpcSystemBusServiceImpl implements HpcSystemBusService {
		...
		for (HpcDataObject dataObject : dataObjectsReceived) {
			...
			try {
				...
				HpcDataObjectUploadResponse uploadResponse = dataTransferService.uploadDataObject(
						systemGeneratedMetadata.getSourceLocation(), null, false, path,
			...
			try {
				...
				case ARCHIVED:
					...
					metadataService.updateDataObjectSystemGeneratedMetadata(path, null, null, null, dataTransferStatus,
							null, null, dataTransferCompleted);
					break;
		...
		for (HpcDataObject dataObject : dataObjectsInProgress) {
			...
			try {
				...
				HpcPathAttributes archivePathAttributes = dataTransferService.getPathAttributes(
						...
						false, systemGeneratedMetadata.getConfigurationId());
				if (archivePathAttributes.getExists() && archivePathAttributes.getIsFile()) {
					...
					if (generatedURLExpired(systemGeneratedMetadata.getDataTransferStarted(),
						...
						// add an event.
						metadataService.updateDataObjectSystemGeneratedMetadata(path, null, null, null,
								HpcDataTransferUploadStatus.URL_EXPIRED, null, null, null);

						addDataTransferUploadEvent(systemGeneratedMetadata.getRegistrarId(), path,
								HpcDataTransferUploadStatus.URL_EXPIRED, null, null,
								systemGeneratedMetadata.getDataTransferType(),
								systemGeneratedMetadata.getConfigurationId());
					}
				}
	...
	public void processTemporaryArchive() throws HpcException {
		...
		for (HpcDataObject dataObject : dataObjectsInTemporaryArchive) {
			...
			try {
				...
				File file = dataTransferService.getArchiveFile(systemGeneratedMetadata.getDataTransferType(),
	...
	public void completeDataObjectDownloadTasks() throws HpcException {
		...
		for (HpcDataObjectDownloadTask downloadTask : dataTransferService
				.getDataObjectDownloadTasks(HpcDataTransferType.GLOBUS)) {
			try {
			...
			try {
				...
				if (downloadTask.getType().equals(HpcDownloadTaskType.COLLECTION)) {
					...
					downloadItems = downloadDataObjects(downloadTask.getDataObjectPaths(),
							downloadTask.getDestinationLocation(), downloadTask.getUserId());
				}
		...
		dataManagementService.getBulkDataObjectRegistrationTasks(HpcBulkDataObjectRegistrationTaskStatus.RECEIVED)
				.forEach(listRegistrationTask -> {
					try {
						...
						logger.error(
								"Failed to process a data object list registration: " + listRegistrationTask.getId(),
								e);
	 ...
	 *            The data transfer upload status.
	 * @param sourceLocation
	 *            (Optional) The data transfer source location.
	 * @param dataTransferCompleted
	 *            (Optional) The time the data upload completed.
	 * @param dataTransferType
	 *            The type of data transfer used to upload (Globus, S3, etc).
	...
	private void addDataTransferUploadEvent(String userId, String path, HpcDataTransferUploadStatus dataTransferStatus,
		...
		try {
			switch (dataTransferStatus) {
			case ARCHIVED:
				eventService.addDataTransferUploadArchivedEvent(userId, path, null, sourceLocation,
			...
			case FAILED:
				eventService.addDataTransferUploadFailedEvent(userId, path, null, sourceLocation, dataTransferCompleted,
	 ...
	 * @param userId
	 *            The user ID.
	 * @param registrationTaskId
	 *            The data registration task ID.
	 * @param result
	 ...
	 */
	private void addDataTransferUploadEvent(String userId, String registrationTaskId, boolean result, String message,
			Calendar dataTransferCompleted) {
		try {
			if (result) {
				eventService.addDataTransferUploadArchivedEvent(userId, null, registrationTaskId, null,
						dataTransferCompleted);
			} else {
	 ...
	 *            The user ID.
	 * @param path
	 *            The collection or data objection path.
	 ...
	 *            The download destination location.
	 * @param dataTransferCompleted
	...
	private void addDataTransferDownloadEvent(String userId, String path, HpcDownloadTaskType downloadTaskType,
			String downloadTaskId, HpcDataTransferType dataTransferType, String configurationId, boolean result,
	 ...
	 *            The download destination location.
	 * @param userId
	 *            The user ID who requested the collection download.
		...
		for (HpcCollectionListingEntry subCollectionEntry : collection.getSubCollections()) {
			...
			if (subCollection != null) {
				...
				downloadItems.addAll(downloadCollection(subCollection,
						calculateDownloadDestinationFileLocation(destinationLocation, subCollectionPath), userId));
			}
		}
	 ...
	 * 
	 * @param dataObjectPaths
	 *            The list of data object path to download.
	 * @param destinationLocation
	 *            The download destination location.
	 * @param userId
	 *            The user ID who requested the collection download.
	 ...
	 */
	private HpcCollectionDownloadTaskItem downloadDataObject(String path, HpcFileLocation destinationLocation,
			String userId) {
		HpcDownloadRequestDTO dataObjectDownloadRequest = new HpcDownloadRequestDTO();
		dataObjectDownloadRequest.setDestination(calculateDownloadDestinationFileLocation(destinationLocation, path));
	...
	/**
	 * Calculate a download destination path for a collection entry under a
	 * collection.
	 * 
	 * @param collectionDestination
	 *            The collection destination location.
	 * @param collectionListingEntryPath
	 *            The entry path under the collection to calculate the destination
	 *            location for.
	 ...
	 *
	 * @param downloadTask
	 *            The download task to complete.
	 * @param result
	 *            The result of the task (true is successful, false is failed).
	 * @param message
	 *            (Optional) If the task failed, a message describing the failure.
	 * @throws HpcException
	 *             on service failure.
	 */
	private void completeCollectionDownloadTask(HpcCollectionDownloadTask downloadTask, boolean result, String message)
			throws HpcException {
		...
		addDataTransferDownloadEvent(downloadTask.getUserId(), path, downloadTask.getType(), downloadTask.getId(),
				// TODO: data-transfer-type and DOC needs to be carried in the collection
				// download
				...
				HpcDataTransferType.GLOBUS, "", result, message, downloadTask.getDestinationLocation(), completed);
	}
	 ...
	 * (succeeded ot failed), record the result.
	 *
	 * @param downloadTask
	 *            The download task to complete.
	 * @throws HpcException
	 *             on service failure.
	 */
	private void completeDataObjectDownloadTaskInProgress(HpcDataObjectDownloadTask downloadTask) throws HpcException {
		...
		if (!dataTransferDownloadStatus.equals(HpcDataTransferDownloadStatus.IN_PROGRESS)) {
			...
			if (downloadTask.getCompletionEvent()) {
				addDataTransferDownloadEvent(downloadTask.getUserId(), downloadTask.getPath(),
						...
						downloadTask.getConfigurationId(), result, message, downloadTask.getDestinationLocation(),
						completed);
			}
		}
	}
	...
	private void deleteDataObject(String path) {
		...
		try {
			metadataService.updateDataObjectSystemGeneratedMetadata(path, null, null, null,
					HpcDataTransferUploadStatus.FAILED, null, null, null);
	 ...
	 *            The data object registration item (one in a list).
	 * @param userId
	...
	private void registerDataObject(HpcBulkDataObjectRegistrationItem registrationItem, String userId) {
		...
		registrationDTO.getParentCollectionMetadataEntries()
	 ...
	 *            The registration task to complete.
	 * @param result
	 *            The result of the task (true is successful, false is failed).
	 * @param message
	 *            (Optional) If the task failed, a message describing the failure.
	...
	private void completeBulkDataObjectRegistrationTask(HpcBulkDataObjectRegistrationTask registrationTask,
		...
		// Send an event.
		addDataTransferUploadEvent(registrationTask.getUserId(), registrationTask.getId(), result, message, completed);
	}
	 ...
	 *
	 * @param registrationItem
	 *            The registration item to check.
	 */
	 ...
	 *
	 * @param urlCreated
	 *            The data/time the URL was generated
	 * @param configurationId
	 *            The data management configuration ID. This is to get the
	 *            expiration config.
	 */
