...
public interface HpcDataManagementBusService {
  ...
  /**
   * Download data objects or collections. Note: API doesn't support mixed, so user expected to
   * provide a list of data objects or a list of collections, not both.
   *
   * @param downloadRequest The download request DTO.
   * @return Download Response DTO.
   * @throws HpcException on service failure.
   ...
   *
   * <p>
   * Note that there is no representation of a user having no permission on a collection. In other
   * words, that scenario is represented by nothing/void/nil.
   *
   * @param collectionPaths The collections' paths.
   ...
   *        data - Specify a source in 'dataObjectRegistrationDTO' or provide this file. The caller
  ...
  /**
   * Get the Data Management Models (Metadata validation rules and hierarchy definitions for all
   * archives)
   *
  ...
  /**
   * Get a Data Management Model (Metadata validation rules and hierarchy definitions) for a
   * specific archive (basePath)
   *
   * @return Data Management Model DTO.
   * @throws HpcException on service failure.
   */
