...
public class HPCCmdRegisterLocalFile extends HPCCmdClient {
	...
	protected boolean processCmd(String cmd, Map<String, String> criteria, String outputFile, String format,
		...
		try {
			...
			if(criteria.get("metadata") != null && criteria.get("metadata").equalsIgnoreCase("true"))
				metadataOnly = true;
			
			if(criteria.get("test") != null && criteria.get("test").equalsIgnoreCase("true"))
				testRun = true;

			if(criteria.get("confirm") != null && criteria.get("confirm").equalsIgnoreCase("true"))
				confirmation = true;
			...
			try {
				...
				success = generator.run(localPath, excludePattern, includePattern, filePathBaseName, destinationBasePath, logFile, logRecordsFile, testRun, confirmation, metadataOnly);
				logRecordsFile = null;
			} catch (Exception e) {
				createErrorLog();
				success = false;
